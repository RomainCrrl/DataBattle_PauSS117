...
```
Ensuite, il vous faudra installé venv afin par la suite de créer un environnement virtuel.

```bash
sudo apt-get install python3-venv
```
venv installé, vous allez maintenant pouvoir procéder à la création de cet environnement virtuel. Pour cela, navigué jusqu'a votre environnement de travail et tapez la commande suivante :
